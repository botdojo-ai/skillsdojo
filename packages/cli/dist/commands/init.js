...
import { saveWorkspaceConfig, saveWorkspaceIndex, getWorkspaceConfig } from '../lib/config.js';
export const initCommand = new Command('link')
    ...
    if (collResponse.error || !collResponse.data) {
        if (options.create) {
            ...
            const refetch = await api.getCollectionBySlug(createResponse.data.account?.slug || accountSlug, collectionSlug);
            if (refetch.data) {
...
function initWorkspace(cwd, collection, collectionPath, spinner) {
    const [accountSlug, collectionSlug] = collectionPath.split('/');
