...
import { findWorkspaceRoot, getWorkspaceConfig, getWorkspaceIndex, saveWorkspaceIndex, saveWorkspaceConfig, } from '../lib/config.js';
...
export const pullCommand = new Command('pull')
    ...
    const changes = [];
    if (added > 0)
        changes.push(chalk.green(`${added} added`));
    if (updated > 0)
        changes.push(chalk.yellow(`${updated} updated`));
    if (deleted > 0)
        changes.push(chalk.red(`${deleted} deleted`));
    if (changes.length > 0) {
