...
function generateDefaultTitle(changes) {
    ...
    const parts = [];
    if (newCount > 0)
        parts.push(`add ${newCount} skill${newCount > 1 ? 's' : ''}`);
    if (modifiedCount > 0)
        parts.push(`update ${modifiedCount} skill${modifiedCount > 1 ? 's' : ''}`);
    if (deletedCount > 0)
        parts.push(`remove ${deletedCount} skill${deletedCount > 1 ? 's' : ''}`);
